...
local MainUIColor = COL3RGB(0, 255, 255)    
...
local gui = library:New("LuckyTap.cc")      
...
local function AddRage(Tab)      
	...
	Tab:Element("Slider", "minimum damage", {min = -300, max = 30, default = 1})      
	Tab:Element("Slider", "max fov", {min = 1, max = 1020, default = 1020})      
end      
...
for i,v in pairs(game.Players.LocalPlayer.PlayerGui.Animate:GetDescendants()) do 
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "sus", "180", "funny", "bad", "bad2", "bad3", "random", "cool", "cool2", "back"}})  
...
antiaim:Element("Dropdown", "body roll", {options = {"off", "on", "-180", "-360", "spin"}})
...
fakelag:Element("Toggle","debris clear",{},function(tbl)
...
chat:Element("Toggle", "chat spam", nil, function(tbl)      
	if tbl.Toggle then      
		while values.misc.chat["chat spam"].Toggle do      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Lucky tap" and "$$$ LuckyTap.cc on Top $$$" or values.misc.chat.type.Dropdown == "Dev" and "Made by Beluga & Foxbanana" or values.misc.chat.type.Dropdown == "Social Creds" and "😭😭😭 -9999999999999999999999 Social Credits 😭😭😭", false, "Innocent", false, true)  
			wait(values.misc.chat["speed (ms)"].Slider/1000)      
				
		end      
	end      
end)      
chat:Element("Dropdown", "type", {options = {"Lucky tap", "Dev", "Social Creds"}})  
chat:Element("Slider", "speed (ms)", {min = 50, max = 1000, default = 500})      
chat:Element("Toggle", "kill say")      
chat:Element("TextBox", "message", {placeholder = "Get tapped by Luckytap.cc"})      
...
CreateHitElement("Loading LuckyTap.cc...",Color3.new(1,1,0))
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then      
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and 0 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "sus" and -5/93 -13 or values.rage.angles["pitch"].Dropdown == "funny" and 20 or values.rage.angles["pitch"].Dropdown == "bad" and math.huge -5/0 -1 or values.rage.angles["pitch"].Dropdown == "bad2" and 23 or values.rage.angles["pitch"].Dropdown == "cool2" and -13 or values.rage.angles["pitch"].Dropdown == "cool" and 12 or values.rage.angles["pitch"].Dropdown == "back" and -92 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-5674596753, 156783400)/5680985749085 or 2.5  
...
mt.__namecall = function(self, ...)      
   ...
   for i,v in pairs(game:GetChildren()) do
	...
	if method == "FireServer" and self.Name == "HitPart" then
		...
		if values.rage.aimbot["ragebot"].Toggle then      
			args[8] = 1.5      
		end  
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Man Tapped By Paste🤨🤨🤨🤨🤨🤨🤨🤨🤨 ", false, "Innocent", false, true)  
	end      
end)      
...
local __ll=(130-0x7e)/(((0x28f-389)-160)-104)local ___=(-#'This has been Coded by FoxBanana'+((24624/(-#'Im crying help me please.'+(333-0xcf)))-148))-(0x59/89) 
